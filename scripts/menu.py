...
def press_play():
    find_and_click_on('/play.png', .9, s.region_maker(.4, .02, .17, .05), "Play button pressed", None, 2)


def select_game_mode():
    find_and_click_on('/deathmatch.png', .75, s.region_maker(.51, .08, .125, .05), "Game mode selected", '/deathmatch_1.png')


def close_lobby():
    find_and_click_on('/close_lobby.png', .85, s.region_maker(.345, .155, .16, .09), "Lobby closed", '/close_lobby_1.png')
...
def queueing(again):
    ...
    while True:
        try:
            ...
            thr.do_run = False
            thr.join()
            print(Fore.GREEN + "Found!" + Fore.WHITE)
            break
        except TypeError:
...
def check_rewards():
    try:
        ...
        k.press_button('esc')
        print(Fore.GREEN + "Rewards acquired" + Fore.WHITE)
    except TypeError:
        try:
...
def press_play_again():
    ...
    print(Fore.GREEN + '"Play again" pressed' + Fore.WHITE)
...
def find_and_click_on(pic, conf, region, message=None, second_pic=None, amount_of_clicks=None, delay=None):
    x = None
...
def click_on(pic, confidence, message, region, amount_of_clicks=None):
